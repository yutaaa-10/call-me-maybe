...
提供された11個のテストケースでは、すべて正しい関数とパラメータを生成し、100%の精度を達成した。<br>
また、すべての出力が有効なJSONとして解析できることを確認した。<br>
11個のテストケース全体の実行時間を time uv run python -m src で測定したところ、約1分47秒だったため、5分以内という要件を満たしている。<br>
Constrained Decodingによって、各状態で無効なトークンのlogitを -inf に設定している。そのため、単にLLMへ正しいJSONを生成するよう依頼するのではなく、JSON構造・関数名・パラメータ名・値の型を生成時に制限している。<br>
一方で、小規模なLLMの出力には不安定さがあり、未知のregexなど一部のケースでは意味的に不適切な出力が生成される場合がある。<br>

### Challenges faced
本プロジェクトで特に難しかった点の一つは、LLMのTokenが必ずしも1文字ではないことを考慮したConstrained Decodingの実装です。<br>
1つのTokenに複数の文字が含まれる場合があるため、JSONを1文字ずつ生成していると仮定することはできません。<br>
そのため、候補となるTokenを文字列へデコードし、現在までに生成された文字列と組み合わせた上で、そのTokenを許可できるか判定するようにしました。<br>

また、数値をどのタイミングで終了させるかという点も難しかったです。<br>
例えば 144 は有効な数値ですが、その後に小数点が続く可能性もあります。<br>
そこで、「数値として続けられる状態」と「数値として終了できる状態」を分けて判定し、さらに残りのパラメータが存在する場合は ,、最後のパラメータの場合は } を許可するようにしました。<br>

さらに、文字列や正規表現の終了位置を判断することにも苦労しました。文字列を早く終了すると必要な文字が欠け、反対に終了できないと余計な文字列を生成し続けてしまいます。そのため、引用符や改行などをチェックするToken Maskingを実装しました。正規表現については re.compile() を利用して構文的に有効か確認し、終了Tokenの選択を補助するようにしました。

### Testing strategy
実装の正しさを確認するために、まず課題で提供された11個のテストケースを使用し、生成された結果について以下の点を確認しました。

* 出力全体が有効なJSONとして解析できること
* 入力されたすべてのpromptに対して結果が生成されること
* 選択されたfunctionが正しいこと
* parameterの名前と値が正しいこと
* numberやstringなど、parameterの型がfunction definitionと一致していること

提供されたテストだけに依存しないように、未知の入力を使った追加テストも行いました。例えば、異なる数値、大きな数値、提供されたテストには存在しない文字列、異なる表現のpromptなどを入力し、特定のサンプルにハードコーディングされた実装になっていないことを確認しました。

また、入力処理については、存在しないファイル、不正なJSON、Pydanticのモデルと一致しないデータなどを入力し、プログラムが予期せずクラッシュせず、エラーメッセージを表示して終了することを確認しました。

出力については python -m json.tool を使用してJSONとして解析可能であることを確認し、それぞれの結果が prompt、name、parameters の3つのキーを持つことも確認しました。

さらに、実行時間を time uv run python -m src で測定し、11個のテストケースが約1分47秒で完了することを確認しました。
